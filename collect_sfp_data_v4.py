...
switches_df = pd.read_csv("switches_p.csv")
...
def parse_huawei_switch(ip, username, password):
    ...
    try:
        ...
        # -------------------------------------------------------------
        # 2. Extract Switch Name (sysname / CLI Prompt)
        # -------------------------------------------------------------
        prompt = net_connect.find_prompt()
        switch_name = re.sub(r"[<>]|\[|\]", "", prompt).strip()

        # Fallback to sysname configuration command if prompt is generic
        ...
        # -------------------------------------------------------------
        # 3. Get Switch Serial Number (display esn / display elabel)
        # -------------------------------------------------------------
        switch_sn = "Unknown"
        ...
        # -------------------------------------------------------------
        # 4. Check Interface Types for Switch Type (1G vs 10G / 40G)
        # -------------------------------------------------------------
        ...
        # -------------------------------------------------------------
        # 5. Extract Interfaces with SFP Modules Present ONLY
        # -------------------------------------------------------------
        ...
        # Build output rows with Switch Name included
        records = []
        if not ports_data:
            print(f"[!] {ip} ({switch_name}) has NO active SFPs inserted.")
            return []
        else:
            for idx, pdata in enumerate(ports_data):
                record = {
                    "Switch IP": ip if idx == 0 else "",
                    "Switch Name": switch_name if idx == 0 else "",
        ...
        print(
            f"[+] Successfully collected {len(ports_data)} active SFP(s) from {switch_name} ({ip})"
        )
        return records
...
# Process all switches concurrently
all_output_rows = []
...
if all_output_rows:
    ...
    output_df.to_excel("huawei_sfp_inventory_final.xlsx", index=False)
    print(
        "\n[✔] Final dataset with Switch Names generated! Saved to 'huawei_sfp_inventory_final.xlsx'"
    )
else:
